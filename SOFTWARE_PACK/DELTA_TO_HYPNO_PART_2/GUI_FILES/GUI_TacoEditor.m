...
handles.tempo_auto = VG_tempo_auto; % PierMOD: inizializzazione per funzione
                                    % interna PlotSegnali
handles.contador = 0;
...
maxSignalTime = length(handles.signalECG)/VG_fs;
...
% PierMOD: togliamo dipendenza di questa funzione dalla frequenza di
%          campionamento, in modo da poter visualizzare anche ECG acquisiti
%          con fcamp non costante.
...
% PierMOD: vedi qualche riga piu in alto.
% handles.N = handles.tStart + (1:length(handles.signalECG))/fcamp;
plot(handles.tempo_auto,handles.signalECG);
...
run(['GUI_FullScreen.m']); %modificato
...
% PierMOD: il campione corrispondente si prende utilizzando arrotondamento
%          con "ceil" (verificato su un po' di prove). Se dovesse dare
%          errori del tipo "exceeding matrix dimension", tornare a versione
%          precedente (con "round").
% sig_respirogramma(:,2) = VG_sig_respiro(round(sig_tacogramma(:,1)*VG_fs));
sig_respirogramma(:,2) = VG_sig_respiro( ceil(sig_tacogramma(:,1)*VG_fs)+1 );
...
choice = questdlg('Choose the algorithm to run', ...
	'Algorithm Choice', ...
	'Pan Tompkins [Default]','Simplified Pan Tompkins','Hardware-like',...
...
switch choice
    case 'Pan Tompkins [Default]'
		...
		if length(locs) >= 2
			handles.signalTACO(:,1) = VG_tempo_auto(locs(2:end));
			handles.signalTACO(:,2) = diff(VG_tempo_auto(locs));
		end
    case 'Simplified Pan Tompkins'
        handles.signalTACO = pan_tompkins(VG_sig_ECG, VG_sig_respiro, VG_fs, VG_tempo_auto);
    case 'Hardware-like'
