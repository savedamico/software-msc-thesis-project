...
% Last Modified by GUIDE v2.5 04-Jun-2018 16:33:09
...
%per export
save('export_N3.mat','training','group')
%
...
%[C,err,P,logp,coeff]=classify(sample,training,group','mahalanobis');
%figure(); hold on;
%gscatter(sample,C,'rb','.',1,'off');
...
axes(handles.axes3); 
plot(xx,class2.*-1,'linewidth',2,'color','b');
hold on;
...
%per export
save('export_N2.mat','training','group')
%
...
%per export
save('export_REM.mat','training','group')
%
...
save('export_hypno.mat','HYPNO_PRINT');
...
%RESP_exp=RESP_load(start:stop);
HYPNO_DEEP_exp=HYPNO_DEEP(start_2:stop_2);

%mettere nella cartella pantompink from gui
