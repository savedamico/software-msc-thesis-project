function [qrs_amp_ecg_orig,qrs_i_corrected,delay,flip_sign]=pan_tompkin(ecg,fs,gr)
...
% filtering
% flip_sign : 1 --> R peaks recognized as > 0
%            -1 --> R peaks recognized as < 0 --> ecg signal flipped (-ecg)
%% Method :
...
NOISL_buf1 = [];
THRS_buf1 = [];
qrs_i_corrected = []; %PierMOD
qrs_amp_ecg_orig = []; %PierMOD
...
%Threshold based on Central Limit Theorem (requires sample normality)
% gamma = 2;   % 2 --> 95% confidence interval
%Threshold based on Chebyshev Theorem (do not require sample normality)
gamma = 3.2; % 3.2 --> 90% confidence interval

% Define positive and negative thresholds
pos_th = mean(ecg) + gamma*std(ecg);
neg_th = mean(ecg) - gamma*std(ecg);

%Use threshold as a criteria to distinguish between negative or positive
%R peaks
over_pos = sum( ecg>pos_th );
over_neg = sum( ecg<neg_th );

if over_pos >= over_neg
    % Do nothing
    flip_sign = 1;
else
    % Flip ECG signal
...
% PierMOD: Inizializzazione per modifica del codice di aggiustamento
%          posizione picchi R individuati (locs)
% locs = locs-delay;
delay = 0; % PierMOD: for output compatibility with previous scripts.

for i = 1 : length(pks)
   
    % PierMOD : flag to know if a R peak is found (used at the end of this
    %           "for" statement
    R_peak_found=0;
...
THRS_buf1 = [THRS_buf1 THR_SIG1];
 ...
 %% adjust R peak position based on the original signal (to remove filters delay)
    % PierMOD :su suggerimento della prof. Bianchi, utilizziamo pure
    %          la correzione della posizione dei picchi R implementata
    %          nel Pan-Tompkins di Riccardo-Giulia-Pier: compariamo la
    %          posizione dei picchi R trovati con il segnale originale e
    %          andiamo a ricercare il massimo del segnale in un intorno del
    %          picco R individuato.
    
    ncamp_min = round(0.150*fs); % 150 ms rappresenta la durata media di un
                                 % QRS fisiologico, secondo Pan e Tompkins
    span = round(ncamp_min/2);  %Creo una intervallo di ricerca per trovare i reali picchi R sull'ECG di partenza
    
    if R_peak_found
        
        % Estremo inferiore della finestra di ricerca
        minc=qrs_i_raw(end)-span;
        ...
        maxc=qrs_i_raw(end)+span;      
        if maxc>length(ecg), maxc=length(ecg); end %Se l'estremo superiore andasse oltre la fine del segnale ...
        
        % Cerco il picco più alto tra gli estremi della finestra di ricerca
        % (è sufficiente ricercare il
        % massimo, perché l'ecg con picchi R negativi viene riconosciuto
        % automaticamente e ribaltato all'inizio di questa funzione)
        [~,x] = max( ecg(minc:maxc) );
        qrs_i_corrected = [qrs_i_corrected,minc+x-1];
        
        % Calcolo anche l'ampiezza del picco R individuato sul segnale
        % originale, per restituirla in uscita dalla funzione
        qrs_amp_ecg_orig = [qrs_amp_ecg_orig , ecg(qrs_i_corrected(end)) * flip_sign];
    end
end
